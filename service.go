...
func (s *Service) GetUserOrganizations(slackUserID string) []OrgInfo {
	...
	for _, m := range s.data.Indexes.Membership.MembershipIndex[uid] {
		...
		case string(MembershipOrg):
			if !seen[m.Name] {
				orgs = append(orgs, OrgInfo{Name: m.Name, Type: string(OrgTypeOrganization)})
				seen[m.Name] = true
			}
		case string(MembershipTeam):
			if !seen[m.Name] {
				orgs = append(orgs, OrgInfo{Name: m.Name, Type: string(OrgTypeTeam)})
				seen[m.Name] = true
			}
			if rel, exists := teamsIndex[m.Name]; exists {
...
func addAncestryItems(orgs *[]OrgInfo, seen *map[string]bool, ancestry struct {
	...
	for _, name := range ancestry.Orgs {
		if !(*seen)[name] {
			*orgs = append(*orgs, OrgInfo{Name: name, Type: string(OrgTypeOrganization)})
			(*seen)[name] = true
		}
	}
	for _, name := range ancestry.Pillars {
		if !(*seen)[name] {
			*orgs = append(*orgs, OrgInfo{Name: name, Type: string(OrgTypePillar)})
			(*seen)[name] = true
		}
	}
	for _, name := range ancestry.TeamGroups {
		if !(*seen)[name] {
			*orgs = append(*orgs, OrgInfo{Name: name, Type: string(OrgTypeTeamGroup)})
			(*seen)[name] = true
		}
	}
	for _, name := range ancestry.Teams {
		if !(*seen)[name] {
			*orgs = append(*orgs, OrgInfo{Name: name, Type: string(OrgTypeParentTeam)})
			(*seen)[name] = true
		}
	}
}
